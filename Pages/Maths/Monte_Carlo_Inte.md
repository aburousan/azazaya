...
Hi again!\\
So, once again you are here.
Yes or No?\\
**Monte Carlo Method**, statistical method of understanding complex physical or mathematical systems by using randomly generated numbers as input into those systems to generate a range of solutions. The likelihood of a particular solution can be found by dividing the number of times that solution was generated by the total number of trials. By using larger and larger numbers of trials, the likelihood of the solutions can be determined more and more accurately.
...
<div class="row">
  <div class="container">
    ...
    <p>
    <i>Rectangular Boxes under the Surface with randomly chosen position. We will understand what this means in this blog</i>.
    </p>
    <div style="clear: both"></div>      
  </div>
</div>
~~~
We have any numerical methods for integrations like **Trapezoidal Method**, **Simpson's Method**, **Romberg Integration** and many more. These are very fast and efficient. They why do we need this **Monte Carlo Integration**?
...
Let's start with the idea of **Expected Value**. We can think expected value as the **weighted average** of a process.
...
This is the normal average with each element having equal weight of 1. But in most cases doing this doesn't provide too many information. Like we clearly know that age $25$ people contribute the most. So, add this type of information, we introduce **weighted average**, i.e.,~~~ <span style="color:red;font-weight:700">
    Expected Value
</span>~~~.
It is defined as,
$$
...
$$
See the *weight is just the probability of $j$* as **probability informs which values contribute the most**.
...
See for our example above $PMF$ takes discrete values($f(x)$ also takes discrete values some fixed value of $x_i$ are only present). But what our variable is continious?
...
<div class="row">
  <div class="container">
    ...
    <p>
    See the PDF is continuous and PMF is discontinious.
    </p>
    <div style="clear: both"></div>      
  </div>
</div>
...
$$
I have you have understood the concepts upto this point. Two formulas of **expected value** is,
1. $\langle c f(x_i) \rangle = c \langle f(x_i) \rangle$
2. $\langle \sum_i f(x_i) \rangle = \sum_i \langle f(x_i) \rangle$

Another important idea is **\col{red}{Variance}**. Intuitively, **Variance** tells us the spread of our data, i.e., how much random variable $X$ is expected to differ from its expected value. Mathematically, It is given by,
$$
...
It's already too long... So, let's stop here and go straight into the Integrations.
...
Monte Carlo's Method is very simple. The steps are as following:
1. Uniformly sample the region $-1\leq x \leq 1$, i.e., choose let's say $N= 100$ random values from $-1$ to $1$. The sample really don't have to be uniform (will discuss in some later blog).
...
This is the case as $\int_{a}^{b}dx\ p(x) = 1$ (see the Figure below). 
~~~
<div class="row">
  <div class="container">
...
~~~
We define **MC Estimate**, i.e., $E_{N}$ of a as (*Important Sampling* method is used hence divided by $p(x_i)$),
$$
...
$$
Hence, we conclude the proof. But one question may arise, Is it true only for uniform distribution? or it is true for any distribution?\\
...
$$
Here we have assumed each sample is independent of each other. So, we have,
$$
Var(E_N)\propto \frac{1}{N} \to Error \propto \frac{1}{\sqrt{N}}
$$
This implies **\col{green}{Increasing the N, i.e., sample number the error of our method will decrease}**.

But it also tells us **\col{blue}{going from let's say 10 samples to 100 samples will reduce the error more than if we go from let's say 10,000 to 1,00,000}**.
...
Let's use all of what we have learnt in julia language.

We will be using the semi-circle integral. Let's see all the lines one by one. For the example we will use **Distributions** package for using **PDF**. Let's see each steps:
...
So, as we can see clearly that the error drops as the $N$ value increases. But compared to other numerical methods, this one seems a bit weak. Well, this weakness just is there in case of single or some times double varible functions , i.e., 2D functions.

We can make the code much more faster (although it is already very fast) using parallel computing and many more modern blackmagic of different algorithms & technology. But here we will not discuss those here.

As we go to higher dimensions, this **Monte Carlo** method truly shines. It is free from the *curse of dimensionality*.

__________________________________________________

I hope you all get to know something new or maybe it was a nice refresher to your old memories.

If you have some question, do let me know in the comments (you need github account) or contact me using my using the informations are given in the end of [About Me](/).
